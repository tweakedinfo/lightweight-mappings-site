...
object old extends LevelledGrid { 
  ...
  def name = "ACS Core Body of Knowledge pre-2022"
  ...
  def page(c:Course) = <.div(
    ...
    LevelledGridComponent(old, c, c.structure),  <.hr,
    Common.markdown(
      s"""
...
object v3_2 extends LevelledGrid {

  def name = "ACS CBoK v3.2"
  ...
  val categories = (Professional.values ++ Core.values).toSeq :+ Depth
  ...
  def page(c:Course) = <.div(
    ...
    (for url <- handbookUrl yield
      ...
      LevelledGridComponent(v3_2, c, c.structure),  <("hr"),
      Common.markdown(
        s"""
          |#### How to edit this table 
          |
      ...
      <.ul(
        for e <- cbok.v3_2.categories yield <.li(<.code(s"${e.jsName}"), " ", <.i(e.name))
      ),
      Common.markdown("""
          |
          | Each function takes an int parameter for its level. E.g. `cbok.v3_2.Ethics.level(3)`
          |        |
          |#### How to limit units shown for each course
          |
          |Within `courses.js`, call `limitCourseGridEntries`. This takes an array of degrees, a grid column, and an array of unit codes. e.g.
...
object CBOKjs {
  ...
  val old = (
    (for e <- cbok.old.categories yield e.jsName -> e).toMap +
    ("page" -> ("ACS CBoK pre-2022", "cbok-2022", cbok.old.page))
  ).toJSDictionary

  val v3_2 = (
    (for e <- cbok.v3_2.categories yield e.jsName -> e).toMap +
    ("page" -> ("ACS CBoK v3.2", "cbok-3_2", cbok.v3_2.page))
  ).toJSDictionary
